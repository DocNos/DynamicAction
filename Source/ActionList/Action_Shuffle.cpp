...
FVector UAction_Shuffle::GetShufflePosition(int32 CardIndex, int32 Iteration)
{
	...
	switch (Iteration % 3)
	{
		case 0: // Fan spread
		{
			...
			Position.Z += LiftHeight + (FMath::Sin(RadAngle + PI / 2) * LiftHeight * 0.5f);
			break;
		}

		case 1: // Split into two piles
		{
			...
			Position.Z += LiftHeight + (PileIndex * 0.5f);
		...
		case 2: // Circular wash shuffle
		{
			...
			Position.Z += LiftHeight * FMath::FRandRange(0.8f, 1.2f);
